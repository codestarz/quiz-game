...
export default function MainHome () {
	...
	const { name } = router.query // Fetches the `name` param from the URL
	...
	return (
		<main className='mainHome relative mx-auto w-full min-h-[25rem] flex gap-4 flex-col justify-between items-center px-5 md:px-10 py-20 lg:col-start-2 lg:row-start-1 lg:row-end-3 text-center text-white'>
			<article>
				<h1 className='text-6xl font-smalls w-full uppercase z-10 relative' translate='no'>
					{/* Dynamically get the name from the URL */}
					{(name || 'Quizi').split('').map((letter, index) => (
